...
<template>
    <div class="fivetable">
        ...
        <el-table :data="tableData" style="width: 100%" v-loading="loading">
            ...
            <el-table-column align="center" prop="address" label="Reason" width="220"></el-table-column>
            <el-table-column align="center" prop="address" label="Operate" width="280px">
                <template slot-scope="scoped">
                    <div>
                        <el-button @click="addBtn(scoped.row)" size="mini">Unblock</el-button>
                    </div>
                </template>
            </el-table-column>
        </el-table>
...
<script>
import { getrecommendListApi, removerecommendApi } from '@/api/anchor/index'
export default {
    data () {
        return {
            providerId: "",
    ...
    methods: {
        ...
        getDataList () {
            this.loading = true
            let obj = { ...this.formPage, parmas: { regionCode: this.regionCode, providerCode: this.providerId, type: '4', level: this.GradeInput } }
        ...
        // 操作
        addBtn (row) {
            removerecommendApi({ providerId: row.userId, type: '4' }).then(res => {
                if (res.code == 200) {
