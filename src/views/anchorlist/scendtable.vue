    ...
    <div class="scendtable">
        <div>
            ...
            <el-table :data="tableData" style="width: 100%" v-loading="loading">
                ...
                <el-table-column align="center" prop="address" label="Operate" width="280">
                    <template slot-scope="scoped">
                        <div class="buttons">
                            <el-button @click="removeBtn(scoped.row)" size="mini">Join</el-button>
...
<script>
import { joinListApi, neverRecommendApi, getrecommendListApi } from '@/api/anchor/index'
export default {
    data () {
        return {
            heights: '105px',
    ...
    methods: {
        ...
        getDataList () {
            this.loading = true
            let obj = { ...this.formPage, parmas: {regionCode: this.regionCode, providerCode: this.providerId, type: '2', level: this.GradeInput } }
        ...
        // 操作
        removeBtn (val) {
            joinListApi({ type: '2', providerId: val.userId }).then(res => {
                if (res.code == 200) {
        ...
        recommed (val) {
            neverRecommendApi({ type: '2', providerId: val.userId }).then(res => {
                if (res.code == 200) {
                    this.$message.success('success！')
                    this.getDataList()
                }
