<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="88px">
      ...
      <el-form-item label="最后登入时间" clearable prop="logoTime" label-width="100"> 
      ...
      <!-- <el-form-item> -->
        <el-form-item label="审核时间" clearable prop="examineTime" label-width="100"> 
        <el-date-picker size="mini" style="width: 240px" v-model="queryParams.parmas.examineTime" value-format='yyyy-MM-dd HH:mm:ss' :default-time="['00:00:00', '23:59:59']" type="daterange" range-separator="-" start-placeholder="开始时间" end-placeholder="结束时间">
        </el-date-picker>
      </el-form-item>
      <el-form-item>
    ...
    <el-table v-loading="loading" :data="umsInfos">
      ...
      <el-table-column label="等级" align="center" prop="addTime" width="250">
        <template slot-scope="scoped">
            <span>{{ scoped.row.levelCode | statusFilter(scoped.row.assessStatus) }}</span>
      ...
      <el-table-column label="审核时间" align="center" prop="addTime" width="250" />
    ...
    <!-- 添加资料 -->
    <el-dialog title="" :visible.sync="addData" width="80%" center :close-on-press-escape="false"
      ...
      <div class="video">
        <div>
          <p>原审核视频</p>
          <video width="300px" height="178px" :src="originalUrl" controls="controls" autoplay></video>
          <!-- <p
            @click="daonload(originalUrl, `${providerInfo.userCode}`)">
            下载</p> -->
        </div>
        <div>
          <p>*更新视频（必选）</p>
          ...
          <el-upload class="avatar-uploader" accept="video/mp4" :action="url" v-bind:on-progress="uploadVideoProcess"
            ...
            <video v-if="formData.url != null && !dialogVisible" :src="formData.url"
              class="avatar video-avatar" controls="controls">
              您的浏览器不支持视频播放
            </video>
            <i v-else-if="formData.url == null && !dialogVisible"
              class="el-icon-plus avatar-uploader-icon"></i>
            <el-progress v-if="dialogVisible == true" type="circle" v-bind:percentage="videoUploadPercent"
              style="margin-top: 7px"></el-progress>
          </el-upload>
          <!-- <div style="margin-top: 5px" v-show="formData.url != null">
              <div style="position: relative; width: 260px">
                <i class="el-icon-delete" style="position: absolute;right: 50px;top: 4px;
                    z-index: 99;
                    color: red;
                  "
                  @click="delImg(x.id)"></i>
                <video :src="formData.url" controls="controls" style="
                    width: 200px;
                    height: 100px;
                    color: red;
                    margin-left: 10px;
      ...
      <span slot="footer" class="dialog-footer">
        <el-button :disabled="disabledType || photoDisabledType" type="primary" style="width: 250px;height: 70px;font-size: 25px;" @click="determineOk">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
    filters:{
        statusFilter(val, typeT){
            ...
            if(val == 'F') return 'F'
            if(val == 'T') {
                if([0,1].includes(typeT)) {
                    return 'T级检测中'
                } else if ([3].includes(typeT)) {
                    return 'T级检测未通过'
                } else if (!typeT) {
                    return 'T'
                }
    ...
    getVideoList() {
      this.loading = true;
        Object.keys(this.queryParams.parmas).forEach(item => {
            if (['logoTime','examineTime'].includes(item) && !this.queryParams.parmas[item]) {
                this.queryParams.parmas[item] = []
            } else if (!this.queryParams.parmas[item] && !['logonTime','examineTime'].includes(item) && item != 'status') {
      ...
      obj.parmas.addTimeStart = obj.parmas.examineTime[0] ? obj.parmas.examineTime[0] : null
      obj.parmas.addTimeEnd = obj.parmas.examineTime[1] ? obj.parmas.examineTime[1] : null
