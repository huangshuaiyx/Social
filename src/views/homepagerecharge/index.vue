    ...
    <div class="coinsfew" @click="closeDialog">
        <div class="coinsfewialog">
            <div class="coinsfewInfo" @click.stop="commodityclick">
                ...
                <div class="titleNum">{{state.shopdata.name }}</div>
                <div class="speciesImg">
                    ...
                    <div class="speciesTetx" v-show="state.shopdata.tagUrl">
                        ...
                        <!-- <div>{{ percentageTxt }}</div> -->
                    </div>
                </div>
                <div class="moneyNum">
                    <div class="numLeft">{{ state.shopdata.price }}</div>
                    <div class="numRight">{{ state.shopdata.discountPrice }}</div>
...
import { getPhoneType, iphoneappPaycoins, googleappPaycoins } from "@/utils/index"
...
const getdiscountItemInfo = () => {
    networkRequest('v1/wallet/item/discountItem', {}, (res) => {
        console.log(res, '匹配充值优惠');
        if (res.code == 200) {
            if (!res.data) return
...
const getcoins = () => {
    ...
    if (getPhoneType() == "Android") {
        if (state.shopdata.jumpType == 1) {
            ...
            openLoading(false)
            JumpH5('homepagerecharge', "/paychannelcountry", "", JSON.stringify({ ...state.shopdata, type: 'coins' }), true);
        }
    } else {
