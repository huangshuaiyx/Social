        ...
        <el-table :data="tableData" style="width: 100%" :default-sort="{prop: 'Testtimes', order: 'descending'}" v-loading="loading">
            ...
            <el-table-column prop="nickName" label="Nickname" width="100">
                <template slot-scope="scoped">
                    <span @click="goUserInfo(scoped.row)">{{ scoped.row.nickName }}</span>
                </template>
            </el-table-column>
            ...
            <el-table-column prop="Operate" label="Operate" width="120px">
                <template slot-scope="scoped">
                            ...
                            <template>
                                <div class="operate">
                                    ...
                                    <el-popconfirm style="margin-left: 5px;" @confirm="failedclick(scoped.row)" title="确定将该主播设置为不通过吗">
                                        <el-link class="ellinkN" style="color: #d9001b;" :underline="false" slot="reference" type="primary">Failed</el-link>
                                    </el-popconfirm>
            ...
            <el-table-column prop="name" label="More" width="120px">
                <template slot-scope="scoped">
                    <span style="cursor: pointer;" @click="govediorecord(scoped.row)">Video Record</span>
                </template>
            </el-table-column>
        </el-table>
        ...
        <el-dialog title="视频记录" :visible="dialogVisible" width="90%" :before-close="handleClose">
            <div>
                <div v-for="(it,index) in vedioList.list" :key="index">
                    <div v-for="(x, i) in it.files " style="cursor:pointer; color:#1890ff; margin-bottom: 10px;">
                        <div :key="x.id">
                            <video width="100%" height="220" controls :src="x.fileUrl"></video>
                        </div>
                    </div>
                </div>
