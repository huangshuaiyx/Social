...
import { networkRequest, getGoodsListSkus, getStartTime, networkRequestburying } from "../api/inedx";
import { Toast } from "vant";
import { nativeEnv } from '../api/inedx'
...
export default new Vuex.Store({
    ...
    mutations: {
        ...
        APPLANGUAGE (state, params) {
            // if()
            state.appLanguage = params.lan ? params.lan : "en";
            console.log(state.appLan, 'params.lan');
    ...
    actions: {
        ...
        async appLanguages (context) {
            return new Promise((resolve) => {
                window.socailNR.call("nativeEnv", {}, (res) => {
                    console.log(res, '语言');
                    resolve(res)
                    context.commit("APPLANGUAGE", res)
                })
            })
        ...
        onStatistics (context, x) {
            getStartTime(res => {
                let time = res.time
                console.log('客户端进入h5的time', time, x.totalCost);
                let cost =
                ...
                networkRequestburying('aaHHuE3luYH0NS0M57iQ4CgnHy13bOFMOFprlZVSeWo2sTSARb8Z0p3EssH9dI7c', params)
            })
