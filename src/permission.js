...
const whiteList = ['/login', '/auth-redirect', '/bind', '/register','/indexv2']
...
router.beforeEach((to, from, next) => {
  ...
  if (getToken()) {
    ...
    if (whiteList.indexOf(to.path) !== -1) {
      ...
      next(`/login?redirect=${Buffer.from(to.fullPath).toString('base64')}`) // 否则全部重定向到登录页
      NProgress.done()
    }
  }
})
...
router.afterEach(() => {
  NProgress.done()
//   if(!document.getElementsByClassName('sidebar-container')[0]) return
//   if(!document.getElementsByClassName('sidebar-resize')[0]) return
